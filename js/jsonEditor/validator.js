...
function validateSchema (schema, value, path) {
    ...
    if (schema['extends']) {
        for (i = 0; i < schema['extends'].length; i++) {
            errors = errors.concat(validateSchema(schema['extends'][i], value, path))
        }
    }
    ...
    if (schema.allOf) {
        for (i = 0; i < schema.allOf.length; i++) {
            errors = errors.concat(validateSchema(schema.allOf[i], value, path))
        }
    }
    ...
    if (schema.anyOf) {
        ...
        for (i = 0; i < schema.anyOf.length; i++) {
            if (!validateSchema(schema.anyOf[i], value, path).length) {
                valid = true
                break
            }
        }
    ...
    if (schema.oneOf) {
        ...
        for (i = 0; i < schema.oneOf.length; i++) {
            ...
            var tmp = validateSchema(schema.oneOf[i], value, path)
            if (!tmp.length) {
                valid++
            }
    ...
    // `not`
    if (schema.not) {
        if (!validateSchema(schema.not, value, path).length) {
            errors.push({
                path: path,
                property: 'not',
    ...
    if (schema.type) {
        ...
        if (Array.isArray(schema.type)) {
            ...
            for (i = 0; i < schema.type.length; i++) {
                if (checkType(schema.type[i], value)) {
                    valid = true
                    break
                }
            }
            ...
            } else if (!checkType(schema.type, value)) {
                errors.push({
                    path: path,
                    property: 'type',
    ...
    if (schema.disallow) {
        ...
        if (Array.isArray(schema.disallow)) {
            ...
            for (i = 0; i < schema.disallow.length; i++) {
                if (checkType(schema.disallow[i], value)) {
                    valid = false
                    break
                }
            }
            ...
            // Simple type
            if (checkType(schema.disallow, value)) {
                errors.push({
                    path: path,
                    property: 'disallow',
    ...
    if (typeof value === 'number') {
        ...
        if (schema.items) {
            ...
            if (Array.isArray(schema.items)) {
                for (i = 0; i < value.length; i++) {
                    ...
                    // Validate against it
                    if (schema.items[i]) {
                        errors = errors.concat(validateSchema(schema.items[i], value[i], path + '.' + i))
                        ...
                        errors = errors.concat(validateSchema(schema.additionalItems, value[i], path + '.' + i))
                ...
                for (i = 0; i < value.length; i++) {
                    errors = errors.concat(validateSchema(schema.items, value[i], path + '.' + i))
                }
            }
        }
        ...
        for (i in schema.properties) {
            ...
            errors = errors.concat(validateSchema(schema.properties[i], value[i], path + '.' + i))
        }
            ...
            for (i in schema.patternProperties) {
                ...
                for (j in value) {
                    ...
                    if (regex.test(j)) {
                        ...
                        errors = errors.concat(validateSchema(schema.patternProperties[i], value[j], path + '.' + j))
                    }
                }
            }
        }
            ...
            for (i in value) {
                ...
                if (!validatedProperties[i]) {
                    ...
                    if (!schema.additionalProperties) {
                        ...
                        errors = errors.concat(validateSchema(schema.additionalProperties, value[i], path + '.' + i))
                    }
                }
            }
        }
        ...
        if (schema.dependencies) {
            for (i in schema.dependencies) {
                ...
                if (Array.isArray(schema.dependencies[i])) {
                    ...
                    errors = errors.concat(validateSchema(schema.dependencies[i], value, path))
                }
            }
        }
    }
    ...
    if (schema.links) {
        for (var m = 0; m < schema.links.length; m++) {
            if (schema.links[m].rel && schema.links[m].rel.toLowerCase() === 'describedby') {
                ...
                errors = errors.concat(validateSchema(schema, value, path, this.translate))
            }
        }
    }
...
function checkType(type, value) {
    ...
    if (typeof type === 'string') {
        ...
        return !validateSchema(type, value).length
    }
}